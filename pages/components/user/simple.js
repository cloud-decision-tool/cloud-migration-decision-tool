...
export default class extends Component{
  ...
  updateCase = () => {
    ...
    // let privateAvg = 0, publicAvg = 0;
      ...
      {
        try{
            let useranswer = Config.DATA[i].options[answers[i]].answer;
            // let answerWeightage = Config.DATA[i].options[answers[i]].weightage;
            //
            // //  ----------- COND 1 ------------
            ...
            //  ----------- COND 2 ------------
            // if()
            // console.log(" is force ",Config.DATA[i].force)
            let isForce = Config.DATA[i].force;
            if(isForce)
            {
            ...
            // console.log(" Each question ", Config.DATA[i].options[answers[i]].answer, answers[i]);
        }catch(e){
  ...
  render(){
    ...
    let percent = parseInt(((currentIndex-1)/Config.DATA.length)*100);
    ...
    if(this.state.showResults){
      ...
      return <div>
      {!showResults &&
        <div style={sm([styles.maxWid400, styles.selfCenter])}>
        ...
        {Config.DATA.map((item,index)=>{
          return <Question currentOption={this.state.answers} onNext={(questionindex, optionindex)=>{
            ...
            this.setState({ answers : tempA, surveyfinished : true }, ()=> {
              if(this.slider){
                if(currentIndex < Config.DATA.length){
      ...
      {showResults && stepFinancial &&
        ...
        >
          <div style={{ overflowY: 'scroll',height: '100%'}}>
            ...
            <p style={sm([  ])}>{ Config.DATA[currentIndex-1] ? Config.DATA[currentIndex-1].help : ""}</p>

            {Config.DATA[currentIndex-1]['video'] && this.state.helpModal &&
            <YouTube
              videoId={Config.DATA[currentIndex-1]['video']}
              opts={{
                height: '300',
                width: '100%',
              }}
